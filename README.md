...
- Urls: 網路資源位址相關設定
  ...
  - PreData: 昨日資訊相關網址
    ...
    - backup: 備用網址(鉅亨網舊版)
  - Capital: 個股資訊
    - enabled: 是否只更新資本額資訊而不交易(`true / false`)
      - `true`: 更新資本額
      - `false`: 交易
    - source: 來源網址(公開資訊觀測站)
...
- `驗證網站憑證`解釋:
  - `true`: 開啟驗證，在建立連線時自動根據網站提供之TLS版本對其憑證進行驗證。
  - `false`: 關閉驗證，在建立連線時將不驗證網站憑證，將有安全性風險。
  - **請只在遇到連線驗證例外狀況時關閉驗證。**
- `更新模式`參數解釋:
  ...
  - `Auto`: 此模式會去搜尋前一個交易日的日誌檔，並判斷該日誌中是否有更新昨日資訊的紀錄。
    - 如果有，等於`Off`
    - 如果沒有，等於`On`
  - `Off`: 不會進行更新，而是直接使用紀錄檔中的昨日成交量與漲停價。
- `更新模式`的字串不看大小寫，如果參數不是上述所列的3種，將套用`Off`。
- **`Auto`模式會需要查看先前日誌檔中幾個訊息，因此`verbose`需要設為0或1才能使用該模式。**
- 關於更新資本額:
  - 由於觀測站的限制，1分鐘只能查詢20次，更新一千多個股票會非常久，因此將更新資本額和交易分開。
  - 由於初始化時盤前檔是必要的，因此必須在能取得盤前檔後才能更新資本額。
  - 更新資本額時會自動套用下方`OnlyShare`條件來排除不必要的查詢。
  - 當選擇更新資本額時，更新模式的任何參數效果都等同於`Off`。
